...
class Payment(MISBaseModel):
    ...
    def get_wallet_payments():
        ...
        id, student_wallet_ref, amount, reg_no
        """
        try:
            with MISBaseModel.get_session() as session:
                ...
                return {
                    ...
                    "Count": len(payments),
                    "Total Amount": sum([float(p.amount or 0) for p in payments]),
                    "Payments": [
                        {
                            "id": p.id,
...
class TblStudentWallet(MISBaseModel):
    ...
    amount = db.Column(db.Numeric(12, 2), nullable=True, default=0.00)
...
class TblStudentWalletLedger(MISBaseModel):
    ...
    parent_credit_id = db.Column(
        db.BigInteger,
        db.ForeignKey("tbl_student_wallet_ledger.id"),
        nullable=True,
        index=True
    )
    ...
    def credit_wallet(
        ...
        """
        Record wallet deposit (sales receipt / payment).
    ...
    def _get_available_credits(student_id, session):
        ...
        for credit in credits:
            ...
            remaining = credit.original_amount + used
    ...
    def apply_wallet_to_invoice(
        ...
        with TblStudentWalletLedger.get_session() as session:
            ...
            credits = TblStudentWalletLedger._get_available_credits(
                student_id, session
            )
    ...
    def get_wallet_balance(student_id):
        """
        Total wallet balance (credits − debits).
        """
        ...
        with TblStudentWalletLedger.get_session() as session:
            balance = (
                ...
                .filter(TblStudentWalletLedger.student_id == student_id)
                .scalar()
            )
...
class TblIncomeCategory(MISBaseModel):
        ...
        try:
            with MISBaseModel.get_session() as session:
                cat_data_obj=session.query(TblIncomeCategory).filter(
                    TblIncomeCategory.id == category_id,
    ...
    def get_unsynced_income_count():
        ...
        try:
            with MISBaseModel.get_session() as session:
                count = session.query(TblIncomeCategory).filter(
                    or_(
                        TblIncomeCategory.income_account_status != 1,
                        TblIncomeCategory.income_account_status.is_(None)
                    ),
            ...
            from flask import current_app
            current_app.logger.error(f"Error counting unsynced income categories for income accounts: {str(e)}")
            return 0

    @staticmethod
    def get_total_income_count():
        """Count total income categories"""
        try:
            with MISBaseModel.get_session() as session:
                count = session.query(TblIncomeCategory).count()
                return count
        except Exception as e:
            from flask import current_app
            current_app.logger.error(f"Error counting total income categories: {str(e)}")
            return 0
...
class TblPersonalUg(MISBaseModel):
    ...
    def get_unsynced_students(limit: int = 50, offset: int = 0):
        ...
        try:
            students = (
                TblPersonalUg.query
                .filter(TblPersonalUg.qk_id.is_(None))
                .order_by(TblPersonalUg.per_id_ug.desc())  # REQUIRED for offset safety
                .options(
                    load_only(
                        TblPersonalUg.per_id_ug,
                        TblPersonalUg.reg_no,
